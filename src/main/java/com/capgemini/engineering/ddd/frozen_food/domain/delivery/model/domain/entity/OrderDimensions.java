...
import com.capgemini.engineering.ddd.frozen_food.domain.delivery.model.domain.entity.saleOrder.SaleOrderReplicaRepository;
...
public class OrderDimensions implements Serializable {
    ...
    public OrderDimensions (SaleOrderID saleOrderID){
         ...
         this.orderHeight = 0;
         this.orderWidth = 0;
         this.weightUnit = saleOrderReplicaRepository.withId(saleOrderID).getSaleOrderProducts().get(0).getWeightUnit();
         this.volumeUnit = saleOrderReplicaRepository.withId(saleOrderID).getSaleOrderProducts().get(0).getVolumeUnit();
         this.lengthUnit = saleOrderReplicaRepository.withId(saleOrderID).getSaleOrderProducts().get(0).getHeightUnit();

         for(Product p : saleOrderReplicaRepository.withId(saleOrderID).getSaleOrderProducts()){
